...
void aht10_get_data(I2C_HandleTypeDef *i2c, uint8_t i2c_addr, sensor_typedef *m_sensor)
{
	...
	{
		 raw_RH = ((uint32_t)receive_buff[1] << 12) |
		                 ((uint32_t)receive_buff[2] << 4)  |
		                 (receive_buff[3] >> 4);
		     m_sensor->humidity =(uint32_t) (((float)raw_RH * 100.0f / 1048576.0f)*100);

		        raw_temp = (((uint32_t)receive_buff[3] & 0x0F) << 16) |
		                    ((uint32_t)receive_buff[4] << 8)         |
		                     receive_buff[5];
		     m_sensor->temp =(uint32_t) (((float)raw_temp * 200.0f / 1048576.0f - 50.0f)*100);
	}
}
